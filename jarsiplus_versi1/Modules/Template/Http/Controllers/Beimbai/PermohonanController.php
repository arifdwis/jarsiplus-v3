...
namespace Modules\Template\Http\Controllers\Beimbai;
...
{
    protected $title = 'Permohonan Lomba Inovasi';

    public function __construct(Permohonan $data, Pemohon $pemohon, Indikator $indikator)
    {
        $this->data = $data;
        ...
        $this->entiti = strtolower('beimbai');
    ...
    public function index()
    {
        ...
        return view("$this->view.permohonan.index", compact('data', 'identityComplete'));
    }
    ...
    public function create(Request $request)
    {
        ...
        // Arif & Alfi selalu bisa akses
        $is_arif = me() && in_array(me()->email, ['arifdwi@samarindakota.go.id', 'alfi.haryadi11@gmail.com']);

        // Cek tanggal buka-tutup (ubah tanggal sesuai kebutuhan)
        $is_closed = \Carbon\Carbon::now('Asia/Makassar')->lessThan(\Carbon\Carbon::create(2026, 4, 1, 0, 0, 0, 'Asia/Makassar'));

        if (!$is_arif && $is_closed) {
            // Input permohonan ditutup sementara
            notify()->flash('Input permohonan saat ini ditutup. Silakan menunggu pengumuman pembukaan input kembali.', 'warning');
            return redirect($this->toIndex);
        }

        return view("$this->view.permohonan.create");
    }
    ...
    public function show(Request $request, $kode)
    {
        ...
        return view("$this->view.permohonan.show", compact('data', 'juriComments'));
    }
    ...
    public function detail(Request $request, $uuid)
    {
        ...
        $juriComments = $this->getJuriKomentar($data);
        return view("$this->view.permohonan.detail", compact('data', 'juriComments'));
    }

    public function finish(Request $request, $uuid)
    {
        $data = $this->data::where('uuid', $uuid)->firstOrFail();
        return view("$this->view.permohonan.finish", compact('data'));
    }
    ...
    public function riwayat(Request $request, $uuid)
    {
        ...
        return view("$this->view.permohonan.riwayat", compact('data', 'histori'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $lockKey = 'beimbai_submit_lock_' . me()->id;
        if (session($lockKey)) {
            return redirect($this->toIndex);
        }
        ...
        $pesan = <<<EOT
            *## JARSIPLUS Kota Samarinda ##*
            ...
            {$permohonan->pemohon1->name} baru saja menambahkan Inovasi (Beimbai) baru.
